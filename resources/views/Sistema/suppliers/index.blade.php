                ...
                <div class="card-body">
                    <div class="row col-12 col-md-12 col-lg-12">
                        <div class="col-md-6 col-lg-6">
                            ...
                            <a href="{{route('suppliers.create')}}" class="btn btn-icon icon-left btn-success"><i class="fas fa-plus-square"></i> Novo Registro</a>

                            {{-- BOTÃO PARA GERAR RELATÓRIO --}}
                            <a href="{{route('suppliers.pdf')}}" target="_blank" class="btn btn-icon icon-left btn-outline-danger">
                                <i class="fas fa-file-pdf"></i> Gerar PDF</a>
                        </div>
                        <div class="col-md-6 col-lg-6">
                            {{-- FORMULÁRIO PARA PESQUISAR REGISTRO --}}
                            <form action="{{ route('suppliers.search') }}" method="POST" class="form form-search form-ds">
                                @csrf
                    ...
                    <br style="clear: both" />
                    {{-- FORMULÁRIO PARA PESQUISAR POR DATA --}}
                    ...
                    <table width="100%" class="table table-striped table-hover" id="tableExport">
                        <thead>
                            <tr>
                                ...
                                <th style="width:auto;">Valor total dos exames</th>
                                ...
                                <tr>
                                    <td class="text-center pt-2">
                                        <div class="custom-checkbox custom-control">
                                            <input type="checkbox" data-checkboxes="mygroup" class="custom-control-input" id="{{$supplier->id}}" name="ids[]" value="{{$supplier->id}}">
                                            <label for="{{$supplier->id}}" class="custom-control-label">&nbsp;</label>
                                        </div>
                                    </td>
                                    <td>{{$supplier->id}}</td>
                                    <td>{{$supplier->name}}</td>
                                    <td>{{formatCnpjCpf($supplier->cnpj)}}</td>
                                    <td>
                                        @php
                                            $totalValue = $supplier->additionalExams->sum('exam_fee');
                                        @endphp
                                    ...
                                    <td>
                                        ...
                                        <a href="{{ route('suppliers.pdf.company', $supplier->id) }}?start_date={{ request('start_date') }}&end_date={{ request('end_date') }}" 
                                            class="btn btn-primary" data-toggle="tooltip" style="min-width:43px; margin-bottom: 5px" target="_blank" 
                                            data-original-title="Baixar este item!">
                                        ...
                                        <a id-remove="{{$supplier->id}}" href="javascript:;" class="remove btn btn-outline-danger btn-danger"
