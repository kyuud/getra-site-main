        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4>Gestão de Portfolios - <b>{{$data->title ?? 'Novo'}}</b></h4>
                </div>
                <div class="card-body">

                    @if( isset($data) )
                        {!! Form::model($data, ['route' => ['portfolios.update', $data->id], 'class' => 'form', 'files' => true, 'method' => 'PUT']) !!}
                    @else
                        {!! Form::open(['route' => 'portfolios.store', 'id'=>'formStore', 'class' => 'form', 'files' => true]) !!}
                    @endif
                        ...
                        <div class="form-group">
                            <label>Título</label>
                            {!! Form::text('title', null, ['placeholder' => 'Digite o título do portfolio aqui...', 'class' => "form-control"]) !!}
                            @error('title')<div class="text-danger">{{ "***$message" }}</div>@enderror
                        </div>

                        {{-- <div class="form-group">
                            <label>Url</label>
                            {!! Form::text('url', null, ['placeholder' => 'Digite a url do portfolio aqui...', 'class' => 'form-control']) !!}
                            @error('url')<div class="text-danger">{{ "***$message" }}</div>@enderror
                        </div> --}}

                        <div class="form-group">
                            <label>Descrição</label>
                            {!! Form::textarea('description', null, ['placeholder' => 'Digite a descrição do portfolio aqui...', 'class' => 'form-control']) !!}
                        ...
                        @if( isset($data) )
                            <div class="form-group">
                                ...
                                <img class="form-control image-atual" style="width: 100%; height: 100%"
                                    src='{{ url("uploads/portfolios/$data->image") }}' />
                            </div>
                        @endif
