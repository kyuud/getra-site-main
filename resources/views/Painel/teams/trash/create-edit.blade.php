        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4>Gestão de membros da equipe - <b>{{$data->title ?? 'Novo'}}</b></h4>
                </div>
                <div class="card-body">

                    @if( isset($data) )
                        {!! Form::model($data, ['route' => ['teams.update.trash', $data->id], 'class' => 'form', 'files' => true, 'method' => 'PUT']) !!}
                    @else
                        {!! Form::open(['route' => 'teams.store', 'class' => 'form', 'files' => true]) !!}
                    @endif
                        ...
                        <div class="form-group">
                            <label>Nome</label>
                            {!! Form::text('title', null, ['placeholder' => 'Digite o nome do membro da equipe aqui...', 'class' => 'form-control']) !!}
                            @error('title')<div class="text-danger">{{ "***$message" }}</div>@enderror
                        {{-- </div>

                        <div class="form-group">
                            <label>Url</label>
                            {!! Form::text('url', null, ['placeholder' => 'Digite a url aqui...', 'class' => 'form-control']) !!}
                            @error('url')<div class="text-danger">{{ "***$message" }}</div>@enderror
                        </div>

                        <div class="form-group">
                            <label>Descrição</label>
                            {!! Form::textarea('description', null, ['placeholder' => 'Digite a descrição aqui...', 'class' => 'form-control']) !!}
